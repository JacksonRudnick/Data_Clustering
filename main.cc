...
Data* ReadEnvVariables(int argc, char* argv[]) {
  ...
  return new Data(file_path, num_of_clusters, max_iterations, 
                  num_of_runs, convergence_threshold);
}
...
int main(int argc, char* argv[]) {
  ...
  #if OUT_TO_FILE
  std::ofstream output_stream("outputs/"+((std::string)argv[1])
    .substr(((std::string)argv[1]).find_last_of("/")+1,((std::string)argv[1])
    .find_last_of(".")-((std::string)argv[1]).find_last_of("/")-1)+".output");
